...
from app.models.schemas import CriticalAgentState
...
def conclusion_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in conclusion agent, {intent_data}")
    ...
    **CONFIDENCE CALIBRATION:**
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in conclusion agent while invoking model")
        # return safe fallback
        return {"conclusion_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("conclusion agent successfully generated response")
        return {"conclusion_response": response.content}
    finally:
        logger.debug("conclusion agent node finished execution")