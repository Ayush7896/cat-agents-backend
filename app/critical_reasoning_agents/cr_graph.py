...
# Import all your CR agents (no synthesizer, no general)
...
def route_based_on_intent(state: CriticalAgentState):
    """Route to the appropriate CR agent based on intent"""
    intent = state['intent_metadata'].intent_critical
    print(f"🎯 CR Intent: {intent}")

    # Map verbose intent to agent node name directly
    intent_to_agent = {
        "Identify the conclusion": "conclusion_agent",
        "Identify an entailment (also known as implication)": "implication_agent",
        "Infer what is most strongly supported": "infer_strongly_supported_agent",
        "Identify or infer an issue in dispute": "infer_dispute_agent",
        "Identify the technique": "identify_technique_agent",
        "Identify the role": "role_agent",
        "Identify the principle": "principle_agent",
        "Match the structure": "structure_agent",
        "Identify a flaw": "flaw_agent",
        "Match flaws": "match_flaws_agent",
        "Necessary Assumptions": "necessary_assumptions_agent",
        "Sufficient Assumptions": "sufficient_assumptions_agent",
        "Strengthen the argument": "strengthen_agent",
        "Weaken the argument": "weaken_agent",
        "Identify what is most/least helpful to know": "most_least_helpful_agent",
        "Explain": "explain_agent",
        "Resolve a conflict": "resolve_conflict_agent",
    }

    selected_agent = intent_to_agent.get(intent)
    
    if selected_agent is None:
        # Handle unknown intent - you have two options:
        # Option 1: Raise an error
        raise ValueError(f"Unknown CR intent: {intent}")
        
        # Option 2: Return a default agent (uncomment below if preferred)
        # print(f"⚠️ Unknown CR intent: {intent}, defaulting to conclusion_agent")
        # return "conclusion_agent"
    
    print(f"🎯 Routing to: {selected_agent}")
    return selected_agent  # Return the agent name directly

# Build the graph
checkpointer = InMemorySaver()
graph = StateGraph(CriticalAgentState)

# Add nodes (NO synthesizer, NO general agent)
...
graph.add_conditional_edges(
    ...
    path_map={
        ...
        "implication_agent": "implication_agent",
        "infer_strongly_supported_agent": "infer_strongly_supported_agent",
        "infer_dispute_agent": "infer_dispute_agent",
        "identify_technique_agent": "identify_technique_agent",
        "role_agent": "role_agent",
        "principle_agent": "principle_agent",
        "structure_agent": "structure_agent",
        "flaw_agent": "flaw_agent",
        "match_flaws_agent": "match_flaws_agent",
        "necessary_assumptions_agent": "necessary_assumptions_agent",
        "sufficient_assumptions_agent": "sufficient_assumptions_agent",
        "strengthen_agent": "strengthen_agent",
        "weaken_agent": "weaken_agent",
        "most_least_helpful_agent": "most_least_helpful_agent",
        "explain_agent": "explain_agent",
        "resolve_conflict_agent": "resolve_conflict_agent"
    }
)

# ALL agents go directly to END (no synthesizer in CR subgraph)
graph.add_edge("conclusion_agent", END)
graph.add_edge("implication_agent", END)
graph.add_edge("infer_strongly_supported_agent", END)
graph.add_edge("infer_dispute_agent", END)
graph.add_edge("identify_technique_agent", END)
graph.add_edge("role_agent", END)
graph.add_edge("principle_agent", END)
graph.add_edge("structure_agent", END)
graph.add_edge("flaw_agent", END)
graph.add_edge("match_flaws_agent", END)
graph.add_edge("necessary_assumptions_agent", END)
graph.add_edge("sufficient_assumptions_agent", END)
graph.add_edge("strengthen_agent", END)
graph.add_edge("weaken_agent", END)
graph.add_edge("most_least_helpful_agent", END)
graph.add_edge("explain_agent", END)
graph.add_edge("resolve_conflict_agent", END)

# Compile the workflow
workflow_critical = graph.compile(checkpointer=checkpointer)
