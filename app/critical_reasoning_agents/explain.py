...
def explain_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in explain agent, {intent_data}")
    # prompts
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in explain agent while invoking model")
        # return safe fallback
        return {"explain_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("explain agent successfully generated response")
        return {"explain_response": response.content}
    finally:
        logger.debug("explain agent node finished execution")