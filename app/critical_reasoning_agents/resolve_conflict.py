...
def resolve_conflict_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in resolve conflict agent, {intent_data}")
    # prompts
    ...
    **NEXT STEP RECOMMENDATION:**
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in resolve conflict agent while invoking model")
        # return safe fallback
        return {"resolve_conflict_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("resolve conflict agent successfully generated response")
        return {"resolve_conflict_response": response.content}
    finally:
        logger.debug("resolve conflict agent node finished execution")
