...
def strengthen_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in strengthen agent, {intent_data}")
    # prompts
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in strengthen agent while invoking model")
        # return safe fallback
        return {"strengthen_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("strengthen agent successfully generated response")
        return {"strengthen_response": response.content}
    finally:
        logger.debug("strengthen agent node finished execution")