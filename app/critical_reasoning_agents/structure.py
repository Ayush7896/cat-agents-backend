...
def structure_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in structure agent, {intent_data}")
    # prompts
    ...
    **matches its structure** (not topic). Prioritize speed + precision.
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in structure agent while invoking model")
        # return safe fallback
        return {"structure_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("structure agent successfully generated response")
        return {"structure_response": response.content}
    finally:
        logger.debug("structure agent node finished execution")