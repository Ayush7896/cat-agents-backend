...
def weaken_agent_node(state: CriticalAgentState):
    ...
    logger.info(f"intent in weaken agent, {intent_data}")
    # prompts
    ...
    try:
        ...
        response = model.invoke(all_messages)
         ...
         # log with traceback
        logger.exception("Error in weaken agent while invoking model")
        # return safe fallback
        return {"weaken_response": "Sorry, I had trouble analyzing the passage. Please try again."}
    else:
        logger.info("weaken agent successfully generated response")
        return {"weaken_response": response.content}
    finally:
        logger.debug("weaken agent node finished execution")